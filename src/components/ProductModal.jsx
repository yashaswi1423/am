            ...
            {product.bulk_pricing && product.bulk_pricing.length > 0 && (
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-xl p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs md:text-sm font-bold text-green-800">Bulk Discounts Available!</p>
                </div>
                <div className="space-y-1.5">
                  {product.bulk_pricing.map((bulk, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 shadow-sm">
                      <span className="text-xs md:text-sm font-semibold text-green-700">{bulk.display_text}</span>
                      <span className="text-xs text-green-600">₹{bulk.price_per_unit.toFixed(2)}/piece</span>
                    </div>
                  ))}
                </div>
              </div>
